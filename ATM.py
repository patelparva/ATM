...
class Account(object):
    ...
    def withDraw(self, amount):
        if amount > self.balance:
            ...
            print(
                f'Your withdrawal was successful. You bank balance is {self.balance}')

    def deposit(self, amount):
        self.balance = self.balance+amount
        print(
            f'Your deposit was successful. Your bank balance is {self.balance}')
    ...
    if index != '':
        if all_Accounts[index][3] == atm_pin:
            while selection != 4:
                ...
                print('4. Exit')
                selection = int(
                    input('Select and type in the number from the above given option '))
                ...
                if selection == 1:
                    ...
                    Parva.deposit(deposit_amount)
                elif selection == 3:
                    withdraw_amount = int(
                        input('Enter The Amount You want to WithDraw '))
                    Parva.withDraw(withdraw_amount)
                elif selection == 4:
